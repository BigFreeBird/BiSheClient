...
public class ClientMessageProcessor {
	...
	public MessageTransfered doFinal() throws Exception {
		...
		//4签名信息
		byte[] md=MessageDigest.getInstance("SHA-256").digest(publicMessage.getBytes());
		String sign=user.getUsername()+":"+new String(md);
		...
		byte[] privateKey=Serializor.serialzeTobytes(clientPrivateKey);
		...
		transfer.setPrivateKey(privateKey);
